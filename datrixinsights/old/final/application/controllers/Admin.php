...
function get_partner($id)
{
	...
	$pp = explode(',', (string) $partn->partner);
	$com = explode(',', (string) $partn->complete);
	$term = explode(',', (string) $partn->terminate);
	$quot = explode(',', (string) $partn->quotafull);
	if(empty($pp))
	{
	   $html='	 <div class="talk">
...
function all_reports()
{
    ...
    // Define defaults before any condition
    $date = '';
    $date1 = '';
    $uid = '';
    $pid = '';
    ...
    if($this->input->post('date') != null || $this->input->post('date') != '')
    {
        ...
        $project = $this->input->post('proejct'); // typo: should probably be 'project'
