...
use App\Models\ExpressionImage;
 
class ExpressionImageController extends Controller
{
     protected $arr_values = array(
        'routename'=>'expression-image.', 
        'title'=>'Expression Image', 
        'table_name'=>'expression_image',
        'page_title'=>'Expression Image',
        "folder_name"=>'/expression-image',
        "upload_path"=>'upload/',
        "page_name"=>'expression-image-detail.blade.php',
        "keys"=>'id,name',
    ...
    public function load_data(Request $request)
    {
      ...
      $data_list = ExpressionImage::where(['status' => $status])->orderBy('id',$order_by);
    ...
    {   
        ...
        $row = ExpressionImage::where(["id"=>$id,])->first();
        if(!empty($row))
        {
            return view($this->arr_values['folder_name'].'/form',compact('data','row'));
        }
    ...
    public function update(Request $request)
    {
        ...
        if(empty($id)) $data = new ExpressionImage;
        else $data = ExpressionImage::find($id);
