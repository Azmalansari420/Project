...
class SubSubCategoryController extends Controller
{
     protected $arr_values = array(
        'routename'=>'sub-sub-category.', 
        'title'=>'Sub Sub Category', 
        'table_name'=>'sub_sub_category',
        'page_title'=>'Sub Sub Category',
        "folder_name"=>'/sub-sub-category',
        "upload_path"=>'upload/',
        "page_name"=>'sub-sub-category-detail.blade.php',
        "keys"=>'id,name',
    ...
    public function load_data(Request $request)
    {
      ...
      // echo $status;
        ...
        $data_list = SubSubCategory::where([$this->arr_values['table_name'].'.status' => $status])->orderBy('id',$order_by)
        ...
        ->select($this->arr_values['table_name'] . '.*','category.name as category_name','sub_category.name as sub_category_name');
    ...
    {   
        ...
        $row = SubSubCategory::where(["id"=>$id,])->first();
        if(!empty($row))
        {
            return view($this->arr_values['folder_name'].'/form',compact('data','row'));
        }
    ...
    public function update(Request $request)
    {
        ...
        if(empty($id)) $data = new SubSubCategory;
        else $data = SubSubCategory::find($id);
