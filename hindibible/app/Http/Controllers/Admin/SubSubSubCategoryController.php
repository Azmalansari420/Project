...
class SubSubSubCategoryController extends Controller
{
     protected $arr_values = array(
        'routename'=>'sub-sub-sub-category.', 
        'title'=>'Sub Sub Sub Category', 
        'table_name'=>'sub_sub_sub_category',
        'page_title'=>'Sub Sub Sub Category',
        "folder_name"=>'/sub-sub-sub-category',
        "upload_path"=>'upload/',
        "page_name"=>'sub-sub-sub-category-detail.blade.php',
        "keys"=>'id,name',
    ...
    public function load_data(Request $request)
    {
      ...
      $data_list = SubSubSubCategory::where([$this->arr_values['table_name'].'.status' => $status])->orderBy('id',$order_by)
      ->leftJoin('category as category', 'category.id', '=', $this->arr_values['table_name'] . '.category_id')
        ...
        ->select($this->arr_values['table_name'] . '.*','category.name as category_name','sub_category.name as sub_category_name','sub_sub_category.name as sub_sub_category_name');
    ...
    {   
        ...
        $row = SubSubSubCategory::where(["id"=>$id,])->first();
        if(!empty($row))
        {
            return view($this->arr_values['folder_name'].'/form',compact('data','row'));
        }
    ...
    public function update(Request $request)
    {
        ...
        if(empty($id)) $data = new SubSubSubCategory;
        else $data = SubSubSubCategory::find($id);
    ...
    {
        $id = Crypt::decryptString($request->id);
        $data = SubSubSubCategory::find($id);
        if($data->delete())
        {
            $responseCode = 200;
            $result['status'] = $responseCode;
