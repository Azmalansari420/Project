...
class Welcome extends CI_Controller {
	...
	public function quick_form()
	{
		...
		{
			$data2['type'] = 1;
			$data2['p_name'] = $this->input->post('p_name');
			$data2['quantity'] = $this->input->post('quantity');
			$data2['mobile'] = $this->input->post('mobile');
			$data2['mesurmentunit'] = $this->input->post('mesurmentunit');
			$data2['addeddate'] = date('Y-m-d H:i:s');

			$this->crud->insert('quickenquiry',$data2);
			redirect('thankyou');
			$this->session->set_flashdata('message','<div class="alert alert-success">Form has been successfully Submit.</div>');
		}
	}
	public function quick_form_bottom()
	{
		if(isset($_POST['submit']))
		{
			$data2['type'] = 2;
			...
			$data2['details'] = $this->input->post('details');
			$data2['addeddate'] = date('Y-m-d H:i:s');
