...
$columnHeader = '';  
$columnHeader =  "Student Registration ID" . "\t" ."Student Password" . "\t" . "Student Name" . "\t" . "Course Name" . "\t". "Course Code" . "\t". "Student Father Name" . "\t". "Student Mother Name" . "\t". "Email" . "\t". "Number" . "\t". "Gender" . "\t". "DOB" . "\t". "State" . "\t". "District" . "\t". "City" . "\t". "Address" . "\t". "Branch name" . "\t". "Duration" . "\t". "Admission Date" . "\t"	 ;  
$setData = '';  
...
if(is_array($fetch_user_details) || is_object($fetch_user_details) && !empty($fetch_user_details)){
	...
	foreach ($fetch_user_details as $value) {
		...
		//get branch details
		$branch_name    = id_to_name('branch',$value->user_add_by_id,'b_name');
		
		$Institute   = id_to_name('institutes',$value->institute_id);
		$value = '"' . @$value->reg_id . '"' . "\t".'"' . @$value->password . '"' . "\t".'"' . @$value->first_name . '"' . "\t".'"' . @$a->name . '"' . "\t".'"' . @$a->c_code . '"' . "\t".'"' . @$value->father_name . '"' . "\t".'"' . @$value->mother_name . '"' . "\t".'"' . @$value->email . '"' . "\t".'"' . @$value->mobile . '"' . "\t".'"' . @$ba . '"' . "\t".'"' . @$value->dob . '"' . "\t".'"' . @$state . '"' . "\t".'"' . @$district . '"' . "\t".'"' . @$city . '"' . "\t".'"' . @$value->address . '"' . "\t".'"' . @$branch_name . '"' . "\t".'"' . @$duration . '"' . "\t".'"' . @$value->join_date . '"' . "\n";  
