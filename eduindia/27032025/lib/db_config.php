...
class Config{
    ...
    // PRIVATE $db_user_name = "eduindiaf_dbuserr";
    // PRIVATE $db_user_pass = "rIf5Lsa!Z";
	// PRIVATE $db_name 	  = "eduindiaf_dbname";
	// private $userTbl      = 'kgkgk_edu_india';
	// PUBLIC 	$con 		  = NULL; 
	...
	PRIVATE $db_name 	  = "office_eduindia"; 
	PUBLIC 	$con 		  = NULL; 
   ...
   //FETCH ALL
	...
	//FETCH ALL
	public function all_api($table,$other=NULL){
        if(count($check) >=1 ){
            $condition = " WHERE ". $this->join_and($check); 
        }else{
			$condition = NULL;
        }
        $sql = "select b_name,email,mobile from $table $other";
		$run = $this->con->query($sql);	
		$compare = $run->num_rows; 	
		$arr = array();
		if($compare >= 1){
