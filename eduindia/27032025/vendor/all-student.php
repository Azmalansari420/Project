...
<section class="content" id="ppg">
      <div class="row">
        ...
        <!--<a href="export_to_excel.php" class="btn btn-info btn-sm">-->
        <!--      <i class="fa fa-save"></i> Export To Excel</a>-->
         ...
         <th>Duration</th>  
          ...
          if(isset($_GET['p_s']) && !empty($_GET['p_s'])){
              $gst_list = $con->all_fetch("user_register",array('user_add_by_id'=>$admin_id,'p_s'=>$_GET['p_s'] ),"order by id desc");
          }else{
					$gst_list = $con->all_fetch("user_register",array('user_add_by_id'=>$admin_id ),"order by id desc");
        }
           ...
           <td><?= id_to_name('institutes_courses',$g->institute_course_id);?></td>
				  ...
				  <td>
				      <?php if($student_passed_status=="Passed"){}else{?>
				  <a href="<?php e($page_name);?>?id=<?=$slider_id ;?>" class="btn btn-success btn-xs"><i class="fa fa-pencil-square-o"></i> Edit</a><br/><br/>
				  <?php } ?>
				  
				  <a href="<?php e($view_page_name);?>?id=<?=$slider_id ;?>" class="btn btn-primary btn-xs"><i class="fa fa-eye"></i> View</a>
				  </td>
				  <td><a href="javascript:;" class="btn btn-danger btn-xs" onclick="all_delete(<?=$slider_id;?>,'.delete_slider','delete_services_item_list','ajax_to_php/admin_file','<?=$services_img;?>');"><i class="fa fa-trash-o"></i> Delete</td> 
                </tr>
					<?php } }?>
                </tfoot>
              </table>
