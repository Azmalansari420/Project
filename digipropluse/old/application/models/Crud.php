...
public function wallet($user_id)
{
  ...
  $user = $this->db->get_where("registration",array("id"=>$user_id,))->result_object();
  if(!empty($user))
  {
...
public function wallet_credit_debit($request_id,$user_id,$type,$add_type,$amount,$message)
    {    
      ...
      if($type=="debit")
      {
        $new_amount = (int) $wallet_amount-$amount;
        $this->db->update("registration",array("wallet"=>$new_amount,),array("id"=>$user_id,));      
      }
      else if($type=='credit')
      {
        $new_amount = (int) $wallet_amount+$amount;
