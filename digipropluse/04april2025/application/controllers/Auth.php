...
function login()
	{
		if(isset($_POST['submit']))
		{
			...
			$this->db->where("(email = '$email' OR user_id = '$email')", NULL, FALSE);
			$this->db->where('password', $password);
			$this->db->where('status', 1);
			$row = $this->db->get('registration')->result_object();
			...
			{
			  $this->session->set_flashdata('login_msg','<div class="alert alert-danger">Wrong Email/Code Or Password.</div>');
