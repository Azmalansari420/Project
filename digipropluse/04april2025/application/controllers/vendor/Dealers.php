   ...
   //--check user login or not
	...
	function add()
	{
		...
		if(isset($_POST['submit']))
		{
			...
			$data['role'] = 2;	
			$data['wallet'] = 0;	
			$data['user_id'] = 'DP'.rand(99999,999999);

			$data['emploey_id'] = $this->input->post('emploey_id');			
			...
			$data['plan_per'] = json_encode($this->input->post('plan_per'));
	...
	function listing()
	{
		...
		$data['ALLDATA'] = $this->crud->selectDataByMultipleWhere($this->arr_values['table_name'],array('vendor_id'=>$hospital->id,'role'=>2));
