                        ...
                        <div class="card-body custom-edit-service">
                           <div class="service-fields mb-3">
                              <div class="row">
                                 ...
                                 <div class="col-lg-6">
                                    <div class="mb-3">
                                       <label class="mb-2">Select Employee<span class="text-danger">*</span></label>
                                       <select class="form-select form-control" name="emploey_id" >
                                          <?php
                                          $hospital = vendor_data();
                                          $user = $this->db->get_where('employees',array('status'=>1,'vendor_id'=>$hospital->id))->result_object();
                                          foreach ($user as $key => $value) 
                                             { ?>
                                             <option <?php if($EDITDATA[0]->emploey_id==$value->id) echo "selected"; ?> value="<?=$value->id ?>"><?=$value->name ?></option>
                                             <?php } ?>
                                       </select>
                                    </div>
                                 </div>
                                 ...
                                 <div class="col-lg-6">
                                    <div class="mb-3">
                                       <label class="mb-2">Select Plan<span class="text-danger">*</span></label>
                                          <select class="form-select form-control" id="employeeSelect" name="plan_per[]" multiple="multiple" required>
                                              <?php
                                              $user = $this->db->get_where('product', array('status' => 1))->result_object();
                                              
                                              // Decode the saved values from DB
                                              $plan_per = json_decode($EDITDATA[0]->plan_per); // Ex: ["2","3"]
                                              
                                              foreach ($user as $value) {
                                                  $selected = (in_array($value->id, $plan_per)) ? 'selected' : '';
                                              ?>
                              ...
                              <div class="service-fields mb-3">
                                 <div class="row">
                                    ...
                                    <div class="col-lg-12">
                                       <div class="service-upload">
                                          <i class="fas fa-cloud-upload-alt"></i>
                                          <span>Upload Passbook/Cheque Front Page *</span>
                                          <img id="blah" src="<?php echo base_url($upload_path); ?><?php echo $EDITDATA[0]->passbookfrontpage; ?>" class="img-fluid">

                                          <input  type="hidden" class="form-control" name="oldpassbookfrontpage" value="<?php echo $EDITDATA[0]->passbookfrontpage; ?>">
                                          <input type="file" name="passbookfrontpage" accept="image/jpeg, image/png, image/gif">
                                       </div>
                                    </div>
