                              ...
                              <table class="datatable table table-hover table-center mb-0">
                                 ...
                                 <tbody>
                                    ...
                                    foreach ($ALLDATA as $key => $data) 
                                       {  $i++;
                                          $username = $this->db->get_where('registration',array('id'=>$data->dealer_id))->result_object();
                                       ?>
                                    <tr>
                                       <td><?=$i ?></td>
                                       <td>
                                          <h2 class="table-avatar">
                                             <span class="avatar avatar-sm me-2"><img class="avatar-img" src="<?php echo base_url($upload_path); ?><?php echo $username[0]->image; ?>" alt="product image" onerror="this.src='<?=base_url() ?>media/uploads/1724412456.png'"></span>
                                             Dealer Name:- <?php if(!empty($username))  echo $username[0]->username ?><br>
                                             Dealer Mobile:- <?php if(!empty($username))  echo $username[0]->mobile ?>
                                          </h2>
                                       </td>
                                       <td><?=$data->subject ?></td>
                                       <td><?=$data->message ?></td>
