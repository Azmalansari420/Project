...
{
  $ci =& get_instance();
  $login_data = $ci->session->userdata('vendor');
  if(!empty($login_data)) 
  {
    $role = $login_data['role'];
    $id = $login_data['user_id'];
    ...
    if(empty($user))
    {
      $ci->session->unset_userdata('vendor');
      redirect(base_url('login'));
    }
  }
  else
...
function chech_dealer_login()
{
  $ci =& get_instance();
  $login_data = $ci->session->userdata('dealer');
  if(!empty($login_data)) 
  {
    $role = $login_data['role'];
    $id = $login_data['user_id'];
    ...
    if(empty($user))
    {
      $ci->session->unset_userdata('dealer');
      redirect(base_url('login'));
    }
  }
  else
...
/*---------------pathology------------*/
function chech_pathology_login()
{
  $ci =& get_instance();
  $login_data = $ci->session->userdata('pathology');
  if(!empty($login_data)) 
  {
    $role = $login_data['role'];
    $id = $login_data['user_id'];
    ...
    if(empty($user))
    {
      $ci->session->unset_userdata('pathology');
      redirect(base_url('login'));
    }
  }
  else
...
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('plans',array('id'=>$id,));
  if(!empty($size))
  {
    $name = $size[0]->name;
  }
...
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('device',array('id'=>$id,));
  if(!empty($size))
  {
    $name = $size[0]->name;
  }
...
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('registration',array('id'=>$id,));
  if(!empty($size))
  {
    $name = $size[0]->username;
  }
  else
  {
    $name = "Not Found..";
...
function withdraw_req($value)
{
  if($value==1)
  {
    $string = '<button class="btn btn-sm btn-primary">Pending</button>';
  }
  else if($value==2)
  {
    $string = '<button class="btn btn-sm btn-primary">Accept</button>';
  }
  else if($value==3)
  {
...
function paymentstatus($value)
{
  ...
  if($value==1)
  {
    $string = '<span class="btn btn-primary btn-sm"> Process</span>';
  }
  else if($value==2)
  {
    $string = '<span class="btn btn-dark btn-sm"> Sucess</span>';
  }
  else if($value==3)
  {
    $string = '<span class="btn btn-danger btn-sm"> Failed</span>';
  }
  return $string;
}
