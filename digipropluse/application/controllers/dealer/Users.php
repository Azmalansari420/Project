...
class Users extends CI_Controller
{
	...
	protected $arr_values = array(
						   	...
						   	'load_path'=>'dealer/users/',
						   	'redirect_path'=>'dealer/users/',
						   	'back_url'=>'users',
						   	'add_url'=>'users',
						   	'edit_url'=>'users',
						   	'delete_url'=>'users',
						   	'view_url'=>'users',
						   	'status_value'=>'users',
						   	'multiple_delete'=>'dealer/users/delete_all',
						   	'controller_name'=>'users',
						   	'page_name'=>'users-doctor-profile.php',
						   ); 
   ...
   //--check user login or not
	...
	function add()
	{
		...
		if(isset($_POST['submit']))
		{
			...
			if($_FILES['passbookfrontpage']['name']!='')
			{
				$bpassbookfrontpage = rand().'.'.explode(".", $_FILES['passbookfrontpage']['name'])[1];
				$path = $this->arr_values['upload_path'].$bpassbookfrontpage;
				move_uploaded_file($_FILES['passbookfrontpage']['tmp_name'],$path);
			}
			else
			{
				$bpassbookfrontpage = "";
			}
			$data['passbookfrontpage'] = $bpassbookfrontpage;	

			if($_FILES['invoice_image']['name']!='')
			{
				$binvoice_image = rand().'.'.explode(".", $_FILES['invoice_image']['name'])[1];
				$path = $this->arr_values['upload_path'].$binvoice_image;
			...
			$data['invoice_image'] = $binvoice_image;	
			
			if($_FILES['kyc_image']['name']!='')
			{
				$bkyc_image = rand().'.'.explode(".", $_FILES['kyc_image']['name'])[1];
				$path = $this->arr_values['upload_path'].$bkyc_image;
			...
			$data['kyc_image'] = $bkyc_image;	
			
			if($_FILES['aadhar_image']['name']!='')
			{
				$baadhar_image = rand().'.'.explode(".", $_FILES['aadhar_image']['name'])[1];
				$path = $this->arr_values['upload_path'].$baadhar_image;
			...
			$data['aadhar_image'] = $baadhar_image;	

			if($_FILES['upload_pancard']['name']!='')
			{
				$bupload_pancard = rand().'.'.explode(".", $_FILES['upload_pancard']['name'])[1];
				$path = $this->arr_values['upload_path'].$bupload_pancard;
			...
			$data['upload_pancard'] = $bupload_pancard;	


			$hospital = dealer_data();
			$data['dealer_id'] = $hospital->id;	
			$data['vendor_id'] = $hospital->vendor_id;	
			$data['role'] = 3;		
			$data['wallet'] = 0;		
	...
	function listing()
	{
		...
		$data['ALLDATA'] = $this->crud->selectDataByMultipleWhere($this->arr_values['table_name'],array('dealer_id'=>$hospital->id,'role'=>3));
	...
	function edit()
	{
		...
		if(isset($_POST['submit']))
		{
			...
			if($_FILES['passbookfrontpage']['name']!='')
			{
				$passbookfrontpage = rand().'.'.explode(".", $_FILES['passbookfrontpage']['name'])[1];
				$path = $this->arr_values['upload_path'].$passbookfrontpage;
				move_uploaded_file($_FILES['passbookfrontpage']['tmp_name'],$path);
			}
			else
			{
				$passbookfrontpage = $_POST['oldpassbookfrontpage'];
			}
			$data['passbookfrontpage'] = $passbookfrontpage;
