...
class Plans extends CI_Controller
{
	...
	protected $arr_values = array(
						   	...
						   	'delete_url'=>'plans',
						   	'view_url'=>'plans',
						   	'table_url'=>'admin/plans/table',
						   	'status_value'=>'plans',
						   	'multiple_delete'=>'admin_con/plans/delete_all',
						   	'pagination_url'=>'admin_con/plans/get_table_data',
   ...
   //--check user login or not
	...
	function add()
	{
		check_controller_inner_access(15,2);
		if(isset($_POST['submit']))
		{
			...
			$getdata = array();
			$get_line = $this->input->post('get_line');
			if(!empty($get_line))
			{
				foreach ($get_line as $key => $value) 
				{
					$getdata[] = array(
						"get_line"=>$get_line[$key],
					);
				}
			}
	...
	function listing()
	{		
		check_controller_inner_access(15,1);
		$data['page_title'] = $this->arr_values['page_title'];
	...
	public function delete()
	  {
	  	check_controller_inner_access(15,4);
	...
	function edit()
	{
		check_controller_inner_access(15,3);		 
		$args=func_get_args();
		if(isset($_POST['submit']))
		{
			date_default_timezone_set('Asia/Kolkata');
	...
	function view()
	{
		check_controller_inner_access(15,5);		 
		$args=func_get_args();
		$data['page_title'] = $this->arr_values['page_title'];
		$data['upload_path'] = $this->arr_values['upload_path'];
		$data['back_url'] = base_url('admin_con/'.$this->arr_values['back_url'].'/listing');
