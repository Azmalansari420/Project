<!DOCTYPE html>
<html lang="en">
<title><?=$page_title ?></title>
   <?php $this->load->view('admin/include/allcss') ?>
   <body>
      <div id="app" class="app app-header-fixed app-sidebar-fixed">
         <?php $this->load->view('admin/include/topbar') ?>
         <?php $this->load->view('admin/include/sidebar') ?>
         <div id="content" class="app-content">
            <h1 class="page-header add-header"><?=$page_title ?> </h1>
            <form class="row" method="post" enctype="multipart/form-data" action="<?=base_url($formURL) ?>">

               <input type="hidden" name="id" value="<?=@$EDITDATA->id ?>">

               <div class="col-lg-8">
                  <div class="card m-b-15">
                     <div class="row card-body">

                        <div class="col-6 form-group ">
                           <label>Select Category</label>
                           <select class="selectpicker  form-control" required name="cat_id" data-style="btn-default" data-live-search="true" >
                              <?php
                              $cate = $this->db->get_where('categories',array('status'=>1))->result_object();
                              foreach($cate as $data)
                                 { ?>
                              <option value="<?=$data->id ?>"  <?= (isset($EDITDATA->cat_id) && $EDITDATA->cat_id == $data->id) ? 'selected' : '' ?>><?=$data->name ?></option>
                              <?php } ?>
                           </select>
                        </div>

                        <div class="col-6 form-group " >
                           <label>Select Type</label>
                           <select class="selectpicker  form-control" required name="type" data-style="btn-default" data-live-search="true" id="typeSelect">
                              <option value="">Select Type</option>
                              <option value="1"  <?= (isset($EDITDATA->type) && $EDITDATA->type == '1') ? 'selected' : '' ?>>Image</option>
                              <option value="2" <?= (isset($EDITDATA->type) && $EDITDATA->type == '2') ? 'selected' : '' ?>>Video</option>
                           </select>
                        </div>

                        <div class="col-12 form-group">
                           <label>Name </label>
                           <input type="text" class="form-control" name="name" value="<?=@$EDITDATA->name ?>" />
                        </div>

                        <div class="col-12 form-group" id="youtubeBox"  style="display: none;">
                           <label>Youtube Link </label>
                           <input type="text" class="form-control" name="youtubelink" value="<?=@$EDITDATA->youtubelink ?>" />
                        </div>
                        
                     </div>
                  </div>
               </div>
               <div class="col-lg-4">
                  <div class="card m-b-15">
                     <div class="row card-body">
                        <div class="col-12 form-group">
                          <label>Click to Upload Image / Video Thumb</label>
                          <input type="file" id="image-input" class="form-control mb-3" name="image" accept="image/*,video/*">

                          <?php 
                          if (!empty($EDITDATA)) {
                              if ($EDITDATA->type == 1) { // type 1 = image
                                  $src = !empty($EDITDATA->image) ? base_url($upload_path . $EDITDATA->image) : '';
                                  ?>
                                  <img id="image-preview" 
                                       src="<?= $src ?>" 
                                       alt="Image Preview" 
                                       width="120" 
                                       style="<?= !empty($EDITDATA->image) ? '' : 'display:none;' ?>">
                                  <?php
                              } else { // type != 1 = video
                                  $src = !empty($EDITDATA->image) ? base_url($upload_path . $EDITDATA->image) : '';
                                  ?>
                                  <img id="image-preview" 
                                       src="<?= $src ?>" 
                                       alt="Image Preview" 
                                       width="120" 
                                       style="<?= !empty($EDITDATA->image) ? '' : 'display:none;' ?>">
                                  <!-- <video id="video-preview" width="200" height="120" controls style="<?= !empty($EDITDATA->image) ? '' : 'display:none;' ?>">
                                    <source src="<?= $src ?>" type="video/mp4">
                                    Your browser does not support the video tag.
                                  </video> -->
                                  <?php
                              }
                          }
                          ?>
                          <!-- Hidden input to keep old file -->
                          <input type="hidden" class="form-control" name="oldimage" value="<?= @$EDITDATA->image; ?>">
                        </div>


                        <div class="col-12 form-group ">
                           <label>Select Status</label>
                           <select class="selectpicker  form-control" required name="status" data-style="btn-default" data-live-search="true" >
                              <option value="1"  <?= (isset($EDITDATA->status) && $EDITDATA->status == '1') ? 'selected' : '' ?>>Show</option>
                              <option value="0" <?= (isset($EDITDATA->status) && $EDITDATA->status == '0') ? 'selected' : '' ?>>Hide</option>
                           </select>
                        </div>
                        <div class="col-12 form-group mt-4">
                           <button type="submit" name="submit" class="btn btn-purple"><?=$page_title ?></button>
                        </div>
                     </div>
                  </div>
               </div>
            </form>
         </div>
         <a href="#" data-click="scroll-top" class="btn-scroll-top fade"><i class="fa fa-arrow-up"></i></a>
      </div>
      <?php $this->load->view('admin/include/theams') ?>
      <?php $this->load->view('admin/include/allscript') ?>

<script>


document.addEventListener("DOMContentLoaded", function() {
  const select = document.getElementById("typeSelect");
  const youtubeBox = document.getElementById("youtubeBox");

  // Function to toggle visibility
  function toggleYoutubeBox() {
    if (select.value === "2") {
      youtubeBox.style.display = "block";
    } else {
      youtubeBox.style.display = "none";
    }
  }

  // Run on load (for edit mode)
  toggleYoutubeBox();

  // Run on change
  select.addEventListener("change", toggleYoutubeBox);
});
</script>
   </body>
</html>