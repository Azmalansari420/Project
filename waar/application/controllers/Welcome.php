...
class Welcome extends CI_Controller {
	...
	{
		$data['EDITDATA'] = $slugdata = $this->crud->selectDataByMultipleWhere('team',array('slug'=>$id,));
		$size_id = $slugdata[0]->id;
	...
	public function service_booking()
	{
		if(isset($_POST['submit']))
		{
			...
			$data2['book_date'] = $this->input->post('book_date');
			$data2['subject'] = $this->input->post('subject');
			$data2['message'] = $this->input->post('message');

			$this->crud->insert('service_booking',$data2);
			redirect('thankyou');
		}
	}
	...
	public function appointment()
	{
		if(isset($_POST))
		{
			...
			$data2['mobile'] = $this->input->post('mobile');
			$data2['email'] = $this->input->post('email');
			$data2['timezone'] = $this->input->post('timezone');
			$data2['select_date'] = $this->input->post('select_date');
			$data2['select_time'] = $this->input->post('select_time');
			$data2['message'] = $this->input->post('message');

			$this->crud->insert('book_appointment',$data2);
			redirect('thankyou');
			$this->session->set_flashdata('message','<div class="alert alert-success">Form has been successfully Submit.</div>');
		}
	}
	...
	public function subscribe_form()
	{
		if(isset($_POST['submit']))
		{
			$data2['name'] = 'Subscriber';
			$data2['email'] = $this->input->post('email');
			$data2['subject'] = 'Subscriber';
