   ...
   <body>
      <div id="app" class="app app-header-fixed app-sidebar-fixed">
         ...
         <div id="content" class="app-content">
            ...
            <form class="row" method="post" enctype="multipart/form-data" action="{{route($addupdate_form)}}">
               @csrf
                        ...
                        <div class="col-md-12">
                            <div class="row" id="add-more-sinfle-image-field">
                                @if (!empty(@$EDITDATA->single_image_data))
                                    @php $getall = json_decode(@$EDITDATA->single_image_data); @endphp
                                    @foreach ($getall as $key => $value)
                                        @include($add_more_singlepage, ['value' => $value,'index' => $key])
                                    @endforeach
                                @else
                                    {{-- Default empty input block --}}
                                    @include($add_more_singlepage, ['value' => null, 'index' => 0])
                                @endif
                            </div>
                        ...
                        <div class="col-md-12">
                           <div class="row" id="add-more-multiple-image-field">
                                @if (!empty($EDITDATA->multiple_images))
                                    @php $getall = json_decode(@$EDITDATA->multiple_images); $count = 1; @endphp
                                ...
                                @else
                                    {{-- Default empty input block --}}
                                    @include('admin/multipleimage/add-multiple-image', ['value' => null, 'count' => 1])
                                @endif
                            </div>
